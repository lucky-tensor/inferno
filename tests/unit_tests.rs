...
use pingora_proxy_demo::{ProxyConfig, ProxyServer, ProxyService, ProxyError, Result};
...
mod config_tests {
    ...
    fn test_configuration_invalid_load_balancing_algorithm() {
        ...
        let result = ProxyConfig::new(config);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("invalid load_balancing_algorithm"));
    }
    ...
    fn test_configuration_from_env_custom_values() {
        ...
        std::env::set_var("PINGORA_BACKEND_SERVERS", "192.168.1.1:8080,192.168.1.2:8080");
    ...
    fn test_configuration_from_env_invalid_values() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Invalid PINGORA_LISTEN_ADDR"));
...
mod error_tests {
    ...
    fn test_error_creation() {
        ...
        let validation_err = ProxyError::request_validation("Bad header", Some("POST /api".to_string()));
        assert!(matches!(validation_err, ProxyError::RequestValidation { .. }));
        ...
        assert!(matches!(unavailable_err, ProxyError::ServiceUnavailable { .. }));
    }
    ...
    fn test_http_status_mapping() {
        ...
        assert_eq!(ProxyError::backend("host", 999, "invalid").to_http_status(), 502); // Invalid code mapped to 502
    ...
    fn test_error_source_chain() {
        ...
        let proxy_error = ProxyError::network("127.0.0.1:8080", "Failed to connect", Some(Box::new(io_error)));
    ...
    fn test_error_conversion_from_addr_parse() {
        let addr_parse_error: std::net::AddrParseError = "invalid_address".parse::<std::net::SocketAddr>().unwrap_err();
...
mod performance_tests {
    ...
    fn test_config_creation_performance() {
        ...
        let duration = start.elapsed();
        assert!(duration < Duration::from_millis(10), "Config creation too slow: {:?}", duration);
    }
    ...
    fn test_metrics_update_performance() {
        ...
        let duration = start.elapsed();
        assert!(duration < Duration::from_millis(50), "Metrics updates too slow: {:?}", duration);
    }
    ...
    fn test_error_creation_performance() {
        ...
        let duration = start.elapsed();
        assert!(duration < Duration::from_millis(20), "Error creation too slow: {:?}", duration);
    }
    ...
    async fn test_server_creation_performance() {
        ...
        let duration = start.elapsed();
        assert!(duration < Duration::from_millis(500), "Server creation too slow: {:?}", duration);
    }
    ...
    fn test_metrics_snapshot_performance() {
        ...
        let duration = start.elapsed();
        assert!(duration < Duration::from_millis(100), "Snapshot creation too slow: {:?}", duration);
    }
    ...
    fn test_prometheus_format_performance() {
        ...
        let duration = start.elapsed();
        assert!(duration < Duration::from_millis(500), "Prometheus format too slow: {:?}", duration);
    }
}