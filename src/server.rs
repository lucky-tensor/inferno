...
use crate::{ProxyConfig, ProxyService, ProxyError, Result};
use crate::metrics::MetricsCollector;
use async_trait::async_trait;
use pingora_core::server::configuration::Opt;
use pingora_core::server::Server;
use pingora_core::services::{listening::Service, ServiceHandle};
...
pub struct ProxyServer {
    ...
    /// Service handle for lifecycle management
...
impl ProxyServer {
    ...
    /// `Err(ProxyError)` with detailed failure information.
    /// 
    /// # Performance Notes
    /// 
    ...
    /// `Err(ProxyError)` if startup or runtime errors occur.
    /// 
    /// # Blocking Behavior
    /// 
    ...
    /// `Err(ProxyError)` if an unrecoverable error occurs.
    #[instrument(skip(self))]
    async fn run_server_loop(&self) -> Result<()> {
        ...
        loop {
            ...
            // In real implementation, this would be handled by Pingora
            break;
        }
    ...
    /// # Returns
    /// 
    /// Returns `Ok(())` if backend is healthy, `Err(ProxyError)` otherwise
    async fn check_backend_health(backend: &SocketAddr, config: &ProxyConfig) -> Result<()> {
        let url = format!("http://{}{}", backend, config.health_check_path);
            ...
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    Err(ProxyError::backend(
                        ...
                        "Health check returned error status".to_string(),
                    ))
                }
            }
            Err(e) => {
    ...
    /// 
    /// This method spawns a background task that serves metrics
    ...
    fn start_metrics_server(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            // serving Prometheus metrics at /metrics endpoint
                    ...
                    Ok::<_, Infallible>(service_fn(move |req: Request<Body>| {
                        ...
                        async move {
                            match (req.method(), req.uri().path()) {
                                (&Method::GET, "/metrics") => {
                                    let snapshot = metrics.snapshot();
                                    let prometheus_data = snapshot.to_prometheus_format();
                                    
                                    Ok::<_, Infallible>(
                                        Response::builder()
                                            .header("content-type", "text/plain; version=0.0.4")
                                            .body(Body::from(prometheus_data))
                                            .unwrap()
                                    )
                                }
                                (&Method::GET, "/health") => {
    ...
    /// or `Err(ProxyError)` if the server is not running.
    /// 
    /// # Examples
    /// 
    /// ```rust
    ...
    pub async fn shutdown(&mut self) -> Result<()> {
        ...
        if let Some(shutdown_tx) = self.shutdown_tx.take() {
            shutdown_tx.send(()).map_err(|_| {
                ProxyError::internal("Failed to send shutdown signal", None)
            })?;
            
            Ok(())
        } else {
            Err(ProxyError::internal("Server is not running", None))
        }
    }
    ...
    /// # Returns
    /// 
    /// Returns `Ok(())` when shutdown completes, or `Err(ProxyError)`
...
mod tests {
    ...
    async fn test_health_check_logic() {
        ...
        match result.unwrap_err() {
            ProxyError::Network { .. } | ProxyError::Timeout { .. } => {
