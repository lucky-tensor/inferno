...
impl MetricsCollector {
    ...
    pub fn record_response(&self, status_code: u16) {
        ...
        match status_code {
            ...
            _ => {
                warn!(status_code = status_code, "Invalid HTTP status code recorded");
            }
        }
    }
    ...
    pub fn record_upstream_selection_time(&self, duration: Duration) {
        ...
        self.upstream_selection_time_us.fetch_add(duration_us, Ordering::Relaxed);
    ...
    pub fn record_backend_connection(&self) {
        ...
        self.active_backend_connections.fetch_add(1, Ordering::Relaxed);
    }
    ...
    pub fn record_backend_connection_error(&self) {
        self.backend_connection_errors.fetch_add(1, Ordering::Relaxed);
        self.active_backend_connections.fetch_sub(1, Ordering::Relaxed);
    }
...
impl MetricsSnapshot {
    ...
    pub fn to_prometheus_format(&self) -> String {
        ...
        // Basic counters
        output.push_str(&format!("# HELP proxy_requests_total Total number of requests received\n"));
        output.push_str(&format!("# TYPE proxy_requests_total counter\n"));
        output.push_str(&format!("proxy_requests_total {}\n", self.total_requests));
        
        output.push_str(&format!("# HELP proxy_requests_active Number of requests currently being processed\n"));
        output.push_str(&format!("# TYPE proxy_requests_active gauge\n"));
        output.push_str(&format!("proxy_requests_active {}\n", self.active_requests));
        
        output.push_str(&format!("# HELP proxy_responses_total Total number of responses sent\n"));
        output.push_str(&format!("# TYPE proxy_responses_total counter\n"));
        output.push_str(&format!("proxy_responses_total {}\n", self.total_responses));
        
        // Status code counters
        output.push_str(&format!("# HELP proxy_responses_by_status_total Responses by HTTP status code\n"));
        output.push_str(&format!("# TYPE proxy_responses_by_status_total counter\n"));
        output.push_str(&format!("proxy_responses_by_status_total{{status=\"2xx\"}} {}\n", self.status_2xx));
        output.push_str(&format!("proxy_responses_by_status_total{{status=\"3xx\"}} {}\n", self.status_3xx));
        output.push_str(&format!("proxy_responses_by_status_total{{status=\"4xx\"}} {}\n", self.status_4xx));
        output.push_str(&format!("proxy_responses_by_status_total{{status=\"5xx\"}} {}\n", self.status_5xx));
        
        // Duration histogram
        output.push_str(&format!("# HELP proxy_request_duration_ms Request duration in milliseconds\n"));
        ...
        output.push_str(&format!("proxy_request_duration_ms_bucket{{le=\"+Inf\"}} {}\n", cumulative));
        
        // Backend metrics
        output.push_str(&format!("# HELP proxy_backend_connections_total Total backend connection attempts\n"));
        output.push_str(&format!("# TYPE proxy_backend_connections_total counter\n"));
        output.push_str(&format!("proxy_backend_connections_total {}\n", self.backend_connections));
        
        output.push_str(&format!("# HELP proxy_backend_connection_errors_total Failed backend connections\n"));
        output.push_str(&format!("# TYPE proxy_backend_connection_errors_total counter\n"));
        output.push_str(&format!("proxy_backend_connection_errors_total {}\n", self.backend_connection_errors));
        
        // Derived metrics
        output.push_str(&format!("# HELP proxy_success_rate Current success rate (0.0-1.0)\n"));
        output.push_str(&format!("# TYPE proxy_success_rate gauge\n"));
        output.push_str(&format!("proxy_success_rate {:.6}\n", self.success_rate()));
        
        output.push_str(&format!("# HELP proxy_requests_per_second Average requests per second\n"));
        output.push_str(&format!("# TYPE proxy_requests_per_second gauge\n"));
        output.push_str(&format!("proxy_requests_per_second {:.2}\n", self.requests_per_second()));
