...
use pingora_proxy_demo::{ProxyConfig, ProxyServer, ProxyError, Result};
use std::env;
...
fn init_logging_with_level(log_level: &str) {
    ...
    if let Err(e) = tracing::subscriber::try_set_global_default(subscriber) {
        warn!("Could not reinitialize logging with configured level: {}", e);
    } else {
        info!(log_level = log_level, "Logging reinitialized with configured level");
    }
}
...
fn print_startup_info(server: &ProxyServer) {
    ...
    println!("   Health checks: {}", if config.enable_health_check { "enabled" } else { "disabled" });
    println!("   TLS encryption: {}", if config.enable_tls { "enabled" } else { "disabled" });
    println!("   Log level: {}", config.log_level);
    
    if config.enable_metrics {
        println!();
        println!("📊 Observability:");
        println!("   Metrics endpoint: http://{}/metrics", config.metrics_addr);
    ...
    if config.enable_metrics {
        println!("   • View metrics:   curl http://{}/metrics", config.metrics_addr);
    }
