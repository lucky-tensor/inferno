    ...
    pub fn from_env() -> Result<Self> {
        ...
        if let Ok(addr) = std::env::var("PINGORA_LISTEN_ADDR") {
            config.listen_addr = addr.parse().map_err(|e| {
                ProxyError::configuration(format!("Invalid PINGORA_LISTEN_ADDR: {}", e), None)
            })?;
        }

        if let Ok(addr) = std::env::var("PINGORA_BACKEND_ADDR") {
            config.backend_addr = addr.parse().map_err(|e| {
                ProxyError::configuration(format!("Invalid PINGORA_BACKEND_ADDR: {}", e), None)
            })?;
        }
        ...
        if let Ok(timeout_str) = std::env::var("PINGORA_TIMEOUT_SECONDS") {
            let timeout_secs: u64 = timeout_str.parse().map_err(|e| {
                ProxyError::configuration(format!("Invalid PINGORA_TIMEOUT_SECONDS: {}", e), None)
            })?;
        ...
        if let Ok(max_conn_str) = std::env::var("PINGORA_MAX_CONNECTIONS") {
            config.max_connections = max_conn_str.parse().map_err(|e| {
                ProxyError::configuration(format!("Invalid PINGORA_MAX_CONNECTIONS: {}", e), None)
            })?;
        }
        ...
        if let Ok(tls_str) = std::env::var("PINGORA_ENABLE_TLS") {
            config.enable_tls = tls_str.parse().map_err(|e| {
                ProxyError::configuration(format!("Invalid PINGORA_ENABLE_TLS: {}", e), None)
            })?;
        }

        if let Ok(metrics_str) = std::env::var("PINGORA_ENABLE_METRICS") {
            config.enable_metrics = metrics_str.parse().map_err(|e| {
                ProxyError::configuration(format!("Invalid PINGORA_ENABLE_METRICS: {}", e), None)
            })?;
        }
        ...
        if let Ok(metrics_addr) = std::env::var("PINGORA_METRICS_ADDR") {
            config.metrics_addr = metrics_addr.parse().map_err(|e| {
                ProxyError::configuration(format!("Invalid PINGORA_METRICS_ADDR: {}", e), None)
            })?;
        }
