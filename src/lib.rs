...
impl ProxyHttp for ProxyService {
    ...
    async fn upstream_peer(
        ...
        let peer = Box::new(HttpPeer::new(
            format!("{}:{}", self.config.backend_addr.ip(), self.config.backend_addr.port()),
        ...
        self.metrics.record_upstream_selection_time(upstream_selection_time);
    ...
    async fn upstream_request_filter(
        ...
        upstream_request: &mut pingora_core::protocols::http::v1::RequestHeader,
        _ctx: &mut Self::CTX,
    ) -> pingora_core::Result<()> {
        // Add proxy identification headers
        upstream_request.insert_header("X-Forwarded-By", "pingora-proxy-demo").unwrap();
        upstream_request.insert_header("X-Proxy-Version", env!("CARGO_PKG_VERSION")).unwrap();
    ...
    async fn upstream_response_filter(
        &self,
        _session: &mut Session,
        upstream_response: &mut pingora_core::protocols::http::ResponseHeader,
        _ctx: &mut Self::CTX,
    ) -> pingora_core::Result<()> {
        ...
        upstream_response.insert_header("X-Proxy-Cache", "MISS").unwrap();
        upstream_response.insert_header("X-Content-Type-Options", "nosniff").unwrap();
        upstream_response.insert_header("X-Frame-Options", "DENY").unwrap();
    ...
    async fn fail_to_proxy(
        ...
        let status_code = match e.etype() {
            pingora_core::ErrorType::ConnectionTimeout | 
            pingora_core::ErrorType::ReadTimeout |
            pingora_core::ErrorType::WriteTimeout => {
            ...
            pingora_core::ErrorType::DNSError => {
                error!(error = %e, "DNS resolution failed for backend");
                502 // Bad Gateway
            }
            _ => {
