...
//! ## Error Categories
//! 
//! - **Configuration Errors**: Invalid proxy configuration
...
/// 
/// This is the standard Result type used throughout the proxy codebase.
...
/// proxy operations. Each variant includes relevant context and is designed
/// to provide actionable information for debugging and monitoring.
/// 
/// ## Design Principles
/// 
...
/// - Error types map clearly to HTTP status codes
#[derive(Error, Debug)]
pub enum ProxyError {
    /// Configuration validation errors
    /// 
    /// These errors occur during proxy startup when validating the
    /// provided configuration. They typically indicate user configuration
    /// mistakes that need to be corrected before the proxy can start.
    /// 
    ...
    /// 
    /// These errors occur when the proxy cannot establish or maintain
    /// network connections to backend servers. They include DNS resolution
    /// failures, connection timeouts, and connection refused errors.
    /// 
    /// **HTTP Status Mapping**: 502 Bad Gateway or 504 Gateway Timeout
    /// 
    /// **Recovery Strategy**: Retry with exponential backoff, try alternative backends
    ...
    /// 
    /// These errors occur when the proxy runs out of system resources
    ...
    /// 
    /// These errors occur when the proxy cannot serve requests due to
...
impl ProxyError {
    ...
    /// ```
    pub fn configuration(message: impl Into<String>, source: Option<Box<dyn std::error::Error + Send + Sync>>) -> Self {
        Self::Configuration {
    ...
    /// ```
    pub fn network(target: impl Into<String>, message: impl Into<String>, source: Option<Box<dyn std::error::Error + Send + Sync>>) -> Self {
        Self::Network {
            target: target.into(),
            message: message.into(), 
            source,
    ...
    /// ```
    pub fn internal(message: impl Into<String>, source: Option<Box<dyn std::error::Error + Send + Sync>>) -> Self {
        Self::Internal {
            message: message.into(),
            source,
        }
    ...
    /// 
    /// This method provides a consistent mapping from internal proxy errors
    ...
    pub fn to_http_status(&self) -> u16 {
        match self {
            ...
            ProxyError::Network { .. } => 502, // Bad Gateway
            ...
            ProxyError::ResourceExhausted { .. } => 503, // Service Unavailable
            ProxyError::RequestValidation { .. } => 400, // Bad Request
            ProxyError::Internal { .. } => 500, // Internal Server Error
            ProxyError::ServiceUnavailable { .. } => 503, // Service Unavailable
        }
    }
    ...
    pub fn is_temporary(&self) -> bool {
        match self {
            ProxyError::Configuration { .. } => false, // Config errors are permanent
            ProxyError::Network { .. } => true,        // Network issues often temporary
            ProxyError::Backend { status, .. } => {
                // 5xx errors are temporary, 4xx are permanent
                *status >= 500
            }
            ProxyError::Timeout { .. } => true,           // Timeouts are temporary
            ProxyError::ResourceExhausted { .. } => true, // Resources may become available
            ProxyError::RequestValidation { .. } => false, // Validation errors are permanent
            ProxyError::Internal { .. } => false,         // Internal errors need investigation
            ProxyError::ServiceUnavailable { .. } => true, // Service may come back
        }
    }
}
...
/// to ProxyError, making error handling more ergonomic when parsing
...
/// 
/// This provides automatic conversion from std::io::Error to ProxyError,
/// mapping common I/O error kinds to appropriate proxy error types.
impl From<std::io::Error> for ProxyError {
    fn from(err: std::io::Error) -> Self {
        match err.kind() {
            std::io::ErrorKind::TimedOut => {
                ProxyError::timeout(std::time::Duration::from_secs(30), "I/O operation")
            }
            std::io::ErrorKind::ConnectionRefused | 
            std::io::ErrorKind::ConnectionAborted |
            std::io::ErrorKind::ConnectionReset => {
                ProxyError::network("unknown", "Connection failed", Some(Box::new(err)))
            }
            std::io::ErrorKind::NotFound => {
                ProxyError::network("unknown", "Host not found", Some(Box::new(err)))
            }
            _ => {
                ProxyError::internal("I/O error", Some(Box::new(err)))
            }
        }
    }
}
...
mod tests {
    ...
    fn test_error_construction() {
        let config_err = ProxyError::configuration("test", None);
        ...
        let backend_err = ProxyError::backend("api.example.com", 500, "error");
    ...
    fn test_http_status_mapping() {
        assert_eq!(ProxyError::configuration("test", None).to_http_status(), 500);
        assert_eq!(ProxyError::network("host", "error", None).to_http_status(), 502);
        assert_eq!(ProxyError::backend("host", 404, "not found").to_http_status(), 404);
        assert_eq!(ProxyError::backend("host", 999, "invalid").to_http_status(), 502);
        assert_eq!(ProxyError::timeout(Duration::from_secs(1), "op").to_http_status(), 504);
        assert_eq!(ProxyError::resource_exhausted("mem", "oom").to_http_status(), 503);
        assert_eq!(ProxyError::request_validation("bad", None).to_http_status(), 400);
        assert_eq!(ProxyError::internal("bug", None).to_http_status(), 500);
        assert_eq!(ProxyError::service_unavailable("down", None).to_http_status(), 503);
    }
    
    #[test]
    fn test_temporary_classification() {
        assert!(!ProxyError::configuration("test", None).is_temporary());
        assert!(ProxyError::network("host", "error", None).is_temporary());
        ...
        assert!(ProxyError::resource_exhausted("mem", "oom").is_temporary());
        assert!(!ProxyError::request_validation("bad", None).is_temporary());
        assert!(!ProxyError::internal("bug", None).is_temporary());
        assert!(ProxyError::service_unavailable("down", None).is_temporary());
    }
    
    #[test]
    fn test_error_conversion() {
        let addr_err: ProxyError = "invalid:address:format".parse::<std::net::SocketAddr>()
            .unwrap_err().into();
        assert!(matches!(addr_err, ProxyError::Configuration { .. }));
        
        let io_err: ProxyError = std::io::Error::from(std::io::ErrorKind::TimedOut).into();
        assert!(matches!(io_err, ProxyError::Timeout { .. }));
    }
}