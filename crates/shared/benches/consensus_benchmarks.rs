...
fn bench_consensus_worst_case(c: &mut Criterion) {
    ...
    group.bench_function("maximum_conflicts", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = resolver
                .resolve_consensus(worst_case_responses.clone())
                .await
                .unwrap();
        });
    });
    ...
    group.bench_function("zero_conflicts", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = resolver
                .resolve_consensus(best_case_responses.clone())
                .await
                .unwrap();
        });
    });
...
fn bench_consensus_realistic_scenarios(c: &mut Criterion) {
    ...
    group.bench_function("small_cluster", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = resolver
                .resolve_consensus(small_cluster.clone())
                .await
                .unwrap();
        });
    });
    ...
    group.bench_function("medium_cluster", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = resolver
                .resolve_consensus(medium_cluster.clone())
                .await
                .unwrap();
        });
    });
    ...
    group.bench_function("network_partition", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = resolver
                .resolve_consensus(partition_scenario.clone())
                .await
                .unwrap();
        });
    });
