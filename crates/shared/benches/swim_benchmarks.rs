...
fn bench_get_live_members(c: &mut Criterion) {
    ...
    for (size, cluster) in clusters {
        group.bench_with_input(
            BenchmarkId::new("get_live_members", size),
            &size,
            |b, _| {
                b.to_async(&rt).iter(|| async {
...
fn bench_swim_statistics(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000].iter() {
        group.bench_with_input(
            BenchmarkId::new("get_stats", size),
            size,
            |b, &size| {
                b.to_async(&rt).iter(|| async {
                    let config = SwimConfig10k::default();
                    let bind_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8100 + size as u16);
                    
                    let (mut cluster, _events) = SwimCluster::new(
                        format!("stats-node-{}", size),
                        bind_addr,
                        config,
                    ).await.unwrap();
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.to_async(&rt).iter(|| async {
                    ...
                    let bind_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8200 + size as u16);
    ...
    for size in [1000, 5000, 10000].iter() {
        group.bench_with_input(
            ...
            |b, &size| {
                b.to_async(&rt).iter(|| async {
                    let config = SwimConfig10k::default();
                    let bind_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8300 + size as u16);
                    
                    let (mut cluster, _events) = SwimCluster::new(
                        format!("memory-node-{}", size),
                        bind_addr,
                        config,
                    ).await.unwrap();
    ...
    for concurrency in [10, 50, 100].iter() {
        group.bench_with_input(
            ...
            |b, &concurrency| {
                b.to_async(&rt).iter(|| async {
                    let config = SwimConfig10k::default();
                    let bind_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8400 + concurrency as u16);
...
fn create_test_peer_info(id: usize) -> PeerInfo {
    PeerInfo {
        ...
        address: format!("127.0.0.{id}:{port}", id = (id % 254) + 1, port = 8000 + (id % 1000)),
        metrics_port: 9090,
