...
use crate::service_discovery::{
    ServiceDiscovery, BackendRegistration, NodeInfo, NodeType
};
...
async fn test_backend_registration_performance() {
    ...
    assert!(duration < Duration::from_millis(100), 
        "Backend registration took {:?}, should be < 100ms", duration);
    
    println!("✓ Backend registration: {:?} (< 100ms requirement)", duration);
}
...
async fn test_backend_list_access_performance() {
    ...
    assert!(!backends.is_empty(), "Should have some healthy backends, got {}", backends.len());
    ...
    assert!(duration < Duration::from_millis(1), 
        "Backend list access took {:?}, should be very fast", duration);
    
    println!("✓ Backend list access: {:?} (< 1ms, spec calls for < 1μs)", duration);
}
...
async fn test_health_check_cycle_performance() {
    ...
    assert!(total_duration < Duration::from_secs(1), 
        "Health check setup took {:?}, should be < 1s", total_duration);
...
async fn test_consensus_resolution_performance() {
    ...
    use crate::service_discovery::{PeerInfo, ConsensusResolver};
    ...
    for response_count in 1..=10 {
        ...
        if response_count >= 5 { break; } // Test with 5 peer responses
    }
    ...
    assert!(duration < Duration::from_millis(50), 
        "Consensus resolution took {:?}, should be < 50ms for small peer sets", duration);
        
    let (peers, metrics) = result.unwrap();
    assert_eq!(peers.len(), 10, "Should resolve to 10 unique peers");
    
    println!("✓ Consensus resolution: {:?} (< 50ms for {} peer responses)", 
...
async fn test_multi_peer_registration_performance() {
    ...
    assert!(duration < Duration::from_millis(10), 
        "Empty peer registration took {:?}, should be < 10ms", duration);
