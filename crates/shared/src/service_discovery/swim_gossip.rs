...
impl SwimGossipManager {
    ...
    pub async fn gossip_update(&self, update: GossipUpdate, priority: GossipPriority) -> ServiceDiscoveryResult<()> {
        ...
        match priority {
            GossipPriority::Critical | GossipPriority::High => {
                self.high_priority_queue.write().await.push_back(propagation);
            }
            GossipPriority::Normal => {
                self.normal_priority_queue.write().await.push_back(propagation);
            }
            GossipPriority::Low => {
    ...
    pub async fn handle_gossip_message(&self, message: GossipMessage) -> ServiceDiscoveryResult<Vec<GossipUpdate>> {
        ...
        if let Some(compressed_data) = &message.full_member_list {
            self.handle_anti_entropy_sync(message.from, compressed_data).await?;
        }
    ...
    pub async fn trigger_anti_entropy_sync(&self, peer_id: u32) -> ServiceDiscoveryResult<()> {
        ...
        let sync_message = GossipMessage {
    ...
    async fn select_gossip_targets(&self, fanout: usize, exclude: &HashSet<u32>) -> Vec<GossipTarget> {
        let members = self.members.read().await;
    ...
    async fn create_gossip_message(&self, max_updates: usize) -> Option<GossipMessage> {
        ...
        let mut contacted_nodes = HashSet::new();
    ...
    async fn handle_anti_entropy_sync(&self, from: u32, compressed_data: &[u8]) -> ServiceDiscoveryResult<()> {
    ...
    fn estimate_message_size(&self, message: &GossipMessage) -> u64 {
        ...
        let sync_size = message.full_member_list.as_ref().map(|l| l.len()).unwrap_or(0);
