...
use super::swim::{SwimCluster, SwimConfig10k, SwimMember, SwimMembershipEvent, SwimStats, MemberState};
...
impl SwimServiceDiscovery {
    ...
    pub async fn new(
        ...
        // Create SWIM cluster
        let (swim_cluster, event_receiver) = SwimCluster::new(
            node_id.clone(),
            bind_addr,
            swim_config,
        ).await?;
        ...
        let legacy_config = super::config::ServiceDiscoveryConfig::default();
        let legacy_service_discovery = Arc::new(ServiceDiscovery::new(legacy_config));
    ...
    pub async fn get_integration_stats(&self) -> SwimIntegrationStats {
        ...
        let cluster = self.swim_cluster.read().await;
        let swim_stats = cluster.get_stats().await;
    ...
    async fn spawn_event_processor(&mut self) -> tokio::task::JoinHandle<()> {
        ...
        let mut event_receiver = std::mem::replace(
    ...
    async fn process_single_event(
        ...
        match event {
            SwimMembershipEvent::MemberJoined(member) => {
                ...
                legacy_service_discovery.register_backend(peer_info).await;
                debug!(node_id = %member.node_id, "Member joined - registered backend");
            }
            
            SwimMembershipEvent::MemberStateChanged { node_id, old_state, new_state } => {
                match (old_state, new_state) {
                    (MemberState::Alive, MemberState::Suspected) => {
                        legacy_service_discovery.mark_backend_unhealthy(&node_id).await;
                        debug!(node_id = %node_id, "Member suspected - marked unhealthy");
                    }
                    (MemberState::Suspected, MemberState::Alive) => {
                        legacy_service_discovery.mark_backend_healthy(&node_id).await;
    ...
    async fn perform_consistency_check(
        ...
        let discovery_backends = legacy_service_discovery.list_backends().await;
        
        // Convert to sets for comparison
        let swim_ids: HashSet<String> = swim_members.iter().map(|m| m.id.clone()).collect();
        let discovery_ids: HashSet<String> = discovery_backends.iter().map(|b| b.id.clone()).collect();
        ...
        if total_inconsistencies > 0 && config.auto_reconciliation {
            ...
            for missing_id in &missing_in_discovery {
                if let Some(member) = swim_members.iter().find(|m| &m.id == *missing_id) {
                    if let Err(e) = legacy_service_discovery.register_backend(member.clone()).await {
                        error!(
                            node_id = %missing_id,
                            error = %e,
