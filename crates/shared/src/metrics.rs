...
//!
//! High-performance metrics collection system for Inferno distributed systems.
...
//! - **System Metrics**: Memory usage, CPU utilization, resource stats
...
/// Thread-safe metrics collector for Inferno operations
///
/// This structure maintains comprehensive metrics about service performance
...
///
/// ```rust
/// use inferno_shared::metrics::MetricsCollector;
/// use std::sync::Arc;
...
impl MetricsCollector {
    ...
    /// a consistent view of service performance at a point in time.
    ///
    /// # Returns
    ///
...
///
/// This structure provides a consistent view of all service metrics
...
impl MetricsSnapshot {
    ...
    ///
    /// ```rust
    /// use inferno_shared::metrics::MetricsSnapshot;
    ...
    pub fn to_prometheus_format(&self) -> String {
        ...
        output.push_str(&format!(
            "# HELP inferno_requests_total Total number of requests received\n"
        ));
        output.push_str(&format!("# TYPE inferno_requests_total counter\n"));
        output.push_str(&format!("inferno_requests_total {}\n", self.total_requests));

        output.push_str(&format!(
            "# HELP inferno_requests_active Number of requests currently being processed\n"
        ));
        output.push_str(&format!("# TYPE inferno_requests_active gauge\n"));
        output.push_str(&format!(
            "inferno_requests_active {}\n",
            self.active_requests
        ));

        output.push_str(&format!(
            "# HELP inferno_responses_total Total number of responses sent\n"
        ));
        output.push_str(&format!("# TYPE inferno_responses_total counter\n"));
        output.push_str(&format!(
            "inferno_responses_total {}\n",
            self.total_responses
        ));

        // Status code counters
        output.push_str(&format!(
            "# HELP inferno_responses_by_status_total Responses by HTTP status code\n"
        ));
        output.push_str(&format!(
            "# TYPE inferno_responses_by_status_total counter\n"
        ));
        output.push_str(&format!(
            "inferno_responses_by_status_total{{status=\"2xx\"}} {}\n",
            self.status_2xx
        ));
        output.push_str(&format!(
            "inferno_responses_by_status_total{{status=\"3xx\"}} {}\n",
            self.status_3xx
        ));
        output.push_str(&format!(
            "inferno_responses_by_status_total{{status=\"4xx\"}} {}\n",
            self.status_4xx
        ));
        output.push_str(&format!(
            "inferno_responses_by_status_total{{status=\"5xx\"}} {}\n",
            self.status_5xx
        ));

        // Duration histogram
        output.push_str(&format!(
            "# HELP inferno_request_duration_ms Request duration in milliseconds\n"
        ));
        output.push_str(&format!("# TYPE inferno_request_duration_ms histogram\n"));
        ...
        for (i, &count) in self.duration_histogram.iter().enumerate() {
            ...
            if i < bucket_boundaries.len() {
                output.push_str(&format!(
                    "inferno_request_duration_ms_bucket{{le=\"{}\"}} {}\n",
                    bucket_boundaries[i], cumulative
                ));
            }
        }
        output.push_str(&format!(
            "inferno_request_duration_ms_bucket{{le=\"+Inf\"}} {}\n",
            cumulative
        ));

        // Backend metrics
        output.push_str(&format!(
            "# HELP inferno_backend_connections_total Total backend connection attempts\n"
        ));
        output.push_str(&format!(
            "# TYPE inferno_backend_connections_total counter\n"
        ));
        output.push_str(&format!(
            "inferno_backend_connections_total {}\n",
            self.backend_connections
        ));

        output.push_str(&format!(
            "# HELP inferno_backend_connection_errors_total Failed backend connections\n"
        ));
        output.push_str(&format!(
            "# TYPE inferno_backend_connection_errors_total counter\n"
        ));
        output.push_str(&format!(
            "inferno_backend_connection_errors_total {}\n",
            self.backend_connection_errors
        ));

        // Derived metrics
        output.push_str(&format!(
            "# HELP inferno_success_rate Current success rate (0.0-1.0)\n"
        ));
        output.push_str(&format!("# TYPE inferno_success_rate gauge\n"));
        output.push_str(&format!(
            "inferno_success_rate {:.6}\n",
            self.success_rate()
        ));

        output.push_str(&format!(
            "# HELP inferno_requests_per_second Average requests per second\n"
        ));
        output.push_str(&format!("# TYPE inferno_requests_per_second gauge\n"));
        output.push_str(&format!(
            "inferno_requests_per_second {:.2}\n",
