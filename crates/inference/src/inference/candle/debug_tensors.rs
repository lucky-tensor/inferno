...
pub fn inspect_safetensors(model_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let model_names: Vec<_> = names.iter().filter(|name| name.starts_with("model.")).collect();
    ...
    println!("\nImportant tensors:");
    let important_names = ["model.embed_tokens.weight", "lm_head.weight", "model.norm.weight"];
    ...
    let head_norm_tensors: Vec<_> = names.iter().filter(|name| name.contains("head") || name.contains("norm")).collect();
    println!("\nHead/Norm tensors:");
...
mod tests {
    ...
    fn inspect_quantized_model_tensors() {
        ...
        let model_path = format!("{}/models/RedHatAI_Llama-3.2-1B-Instruct-quantized.w8a8", home);
