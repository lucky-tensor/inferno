...
use candle_nn::{RmsNorm, Embedding, Linear};
...
impl SimpleQuantizedLinear {
    ...
    pub fn quantized_forward(&self, input: &Tensor) -> Result<Tensor, InferenceError> {
        tracing::debug!("🔥 TRUE quantized forward pass - input shape: {:?}", input.dims());
        ...
        let result = self.quantized_tensor.quantized_matmul(input, activation_scale, activation_zero_point)?;

        tracing::debug!("🔥 Quantized matrix multiplication completed - result shape: {:?}", result.dims());

        // Add bias if present
        if let Some(ref bias) = self.bias {
            result.broadcast_add(bias).map_err(|e| {
                InferenceError::ProcessingError(format!("Failed to add bias: {}", e))
            })
        } else {
            Ok(result)
        }
...
impl HybridQuantizedLlama {
        ...
        for layer_idx in 0..config.num_hidden_layers {
            ...
            for proj in &["q_proj", "k_proj", "v_proj", "o_proj"] {
                ...
                if let Some(qtensor) = quantized_builder.get_quantized_tensor(&tensor_name) {
                    ...
                    quantized_layers.insert(layer_key, SimpleQuantizedLinear::new(qtensor.clone(), None));
                }
            }
    ...
    fn forward_attention(&self, x: &Tensor, layer_idx: usize) -> candle_core::Result<Tensor> {
        ...
        let q = if let Some(q_proj) = self.quantized_layers.get(&q_key) {
            ...
            return Err(candle_core::Error::Msg(format!("Missing quantized Q projection for layer {}", layer_idx)));
        };
        ...
        let k = if let Some(k_proj) = self.quantized_layers.get(&k_key) {
            ...
            return Err(candle_core::Error::Msg(format!("Missing quantized K projection for layer {}", layer_idx)));
        };
        ...
        let v = if let Some(v_proj) = self.quantized_layers.get(&v_key) {
            ...
            return Err(candle_core::Error::Msg(format!("Missing quantized V projection for layer {}", layer_idx)));
        };
        ...
        let k = k.reshape((batch_size, seq_len, num_kv_heads, head_dim))?.transpose(1, 2)?;
        let v = v.reshape((batch_size, seq_len, num_kv_heads, head_dim))?.transpose(1, 2)?;
        ...
        let k = if num_kv_heads != num_heads {
            ...
            let k_repeated = k.unsqueeze(2)? // [batch, num_kv_heads, 1, seq, head_dim]
        ...
        let v = if num_kv_heads != num_heads {
            ...
            let v_repeated = v.unsqueeze(2)? // [batch, num_kv_heads, 1, seq, head_dim]
                .expand(&[batch_size, num_kv_heads, group_size, seq_len, head_dim])? // [batch, num_kv_heads, group_size, seq, head_dim]
                .reshape((batch_size, num_heads, seq_len, head_dim))? // [batch, num_heads, seq, head_dim]
                .contiguous()?; // Make contiguous for matmul
            v_repeated
        ...
        if let Some(o_proj) = self.quantized_layers.get(&o_key) {
            ...
            Err(candle_core::Error::Msg(format!("Missing quantized O projection for layer {}", layer_idx)))
        }
    }
    ...
    fn forward_mlp(&self, x: &Tensor, layer_idx: usize) -> candle_core::Result<Tensor> {
        ...
        let gate_out = if let Some(gate_proj) = self.quantized_layers.get(&gate_key) {
            ...
            return Err(candle_core::Error::Msg(format!("Missing quantized gate projection for layer {}", layer_idx)));
        };
        ...
        let up_out = if let Some(up_proj) = self.quantized_layers.get(&up_key) {
            ...
            return Err(candle_core::Error::Msg(format!("Missing quantized up projection for layer {}", layer_idx)));
        };
        ...
        if let Some(down_proj) = self.quantized_layers.get(&down_key) {
            ...
            Err(candle_core::Error::Msg(format!("Missing quantized down projection for layer {}", layer_idx)))
        }
    }
}