...
impl CandleInferenceEngine {
    ...
    async fn load_tokenizer(&self, model_path: &str) -> Result<Tokenizer, InferenceError> {
        ...
        if tokenizer_path.exists() {
            return Tokenizer::from_file(&tokenizer_path).map_err(|e| {
                InferenceError::InvalidArgument(format!("Failed to load tokenizer: {}", e))
            });
        }
    ...
    /// Create a `VarBuilder` that remaps tensor names from transformer.* to model.*
    #[cfg(any(feature = "candle-cpu", feature = "candle-cuda", feature = "candle-metal"))]
    fn create_remapping_var_builder(base_builder: VarBuilder<'_>) -> VarBuilder<'_> {
        // For now, return the original builder and handle the tensor name mapping
        // in the Llama::load call by using a prefix remapping approach
        base_builder.pp("transformer")
    }
