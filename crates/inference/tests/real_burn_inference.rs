    ...
    pub fn new(config: &SmolLM2Config, device: &B::Device) -> Self {
        ...
        Self {
            q_proj: LinearConfig::new(config.hidden_size, config.num_attention_heads * head_dim)
                .with_bias(false)
                .init(device),
            k_proj: LinearConfig::new(config.hidden_size, config.num_key_value_heads * head_dim)
                .with_bias(false)
                .init(device),
            v_proj: LinearConfig::new(config.hidden_size, config.num_key_value_heads * head_dim)
                .with_bias(false)
                .init(device),
            o_proj: LinearConfig::new(config.num_attention_heads * head_dim, config.hidden_size)
                .with_bias(false)
                .init(device),
